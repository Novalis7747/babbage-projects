...
function pan_initWarnings()
{
	...
	if( messages.length )
	{
		...
		$(warningHome).empty();
		pan_popOverOpenBody($("#jsWarningPopOver"));
	}
}
...
function pan_popOverOpenBody(popOverBody)
{
	...
	leftOffset	= pan_popOverGetLeftOffset( popOverBody );
	if ( leftOffset !== false )
	{ haveOffset = true; offset[ 'left' ] = leftOffset; }

	topOffset	= pan_popOverGetTopOffset( popOverBody );
	if ( topOffset !== false )
...
/*
function pan_initCanvasJs()
{
		var charts = $(".pan_canvasjs:not(.pan_canvasjsDone)");
		pan_console( charts.length + ' charts found' );
		for( i = 0; i < charts.length; i++ )
		{
			var chart = charts[i];
			var data 	= $(chart).data( 'canvasjs' );
			var chartId = $(chart).attr( 'id' );
			pan_console(data);
			var chartJs = new CanvasJS.Chart(chartId,data);
			chartJs.render();
			$(chart).addClass('pan_canvasjsDone');
		}
}
*/
function pan_init()
